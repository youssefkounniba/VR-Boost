...
const stats = [
  { label: "Approved Visits", valeur: 3, icone: CheckCircle2 },
...
const visitAvatars: Record<string, { initiale: string; couleur: string }[]> = {
  "34115": [{ initiale: "A", couleur: "bg-gray-600" }],
  "34116": [
    { initiale: "L", couleur: "bg-amber-500" },
    { initiale: "M", couleur: "bg-blue-500" },
  ],
  "34117": [
    { initiale: "A", couleur: "bg-green-600" },
    { initiale: "K", couleur: "bg-purple-500" },
  ],
};
    ...
    <div>
        ...
        {stats.map((s) => (
          <div key={s.label} className="carte p-5">
            <div className="flex items-start justify-between">
              ...
              <ExternalLink
                className="h-4 w-4 shrink-0 text-brume"
                strokeWidth={1.8}
              />
            </div>
      ...
      <div className="mt-4 space-y-3">
        {projets.slice(0, 3).map((p) => {
          const avatars = visitAvatars[p.id] ?? [];
          return (
            <Link
              key={p.id}
              href={`/projets/${p.id}`}
